<template>
    <div class="effectEditor__linkInput">
        ...
        <button class="addButton" @click="addLink"> Add </button> <button class="clearButton"> Clear </button>

        <ul class="linkList">
            <li class="linkListItem" v-for="(link, index) in value" :key="link.location"> 
                <nuxt-link :to="link.location"> {{ link.title }} </nuxt-link>
                <span class="descriptionSeparator" v-show="link.description"> - </span>
                <span class="description"> {{ link.description }} </span>
                <a class="removeLink" @click="removeLink(index)"> (remove) </a>
            </li>
        </ul>
    </div>
