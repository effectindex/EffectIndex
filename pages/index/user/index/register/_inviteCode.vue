<template>
    <div class="pageContent">
        ...
        <div class="register__formContainer">
            ...
            <div> 
                <label> Password </label> <input v-model="user.password" type="password" />
            </div>
            <div>
                <label> Invite Code </label> <input :value="user.inviteCode" :disabled="user.inviteCode ? true : false" />
            </div>
...
export default {
    ...
    methods: {
        async register() {
            ...
            try {
                let returnedUser = await this.$axios.$post('api/users/register', { user: this.user });
                if (returnedUser && 'user' in returnedUser) this.success = true;
            } catch (error) {
