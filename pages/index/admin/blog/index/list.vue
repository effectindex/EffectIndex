<template>
    <div class="pageContent">
        ...
        <table>
            ...
            <tr v-for="post in $store.state.blogPosts" :key="post._id">
                <td> <nuxt-link :to="'/blog/' + post.slug"> {{ post.title }} </nuxt-link> </td>
                <td> {{ formatDate(post.datetime) }} </td>
                <td> <nuxt-link :to="'/admin/blog/' + post.slug"> [Edit] </nuxt-link>
                <td> <a style="color: red; cursor: pointer;" @click="deletePost(post._id)"> [Delete] </a> </td>
            </tr>
